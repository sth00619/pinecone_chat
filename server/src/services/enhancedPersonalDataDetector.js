...
`;
  ...
  // 범용 월별 일정 조회 (EncryptionService와 통합)
  async getMonthlySchedules(userId, month) {
    try {
      // EncryptionService의 getMonthlySchedules 메서드 사용
      return await this.encryptionService.getMonthlySchedules(userId, month);
    } catch (error) {
      console.error(`Error getting ${month}월 schedules:`, error);
